...
module.exports = {
  all: (req, res) => {
    Gateway.find((err, result, next) => {
      if (!result) res.status(500).send({ status: "Error", message: "no Gateways" })
  ...
  update: (req, res) => {
    if (!ipv4.test(req.body.ipV4))
      res.status(500).send({ status: "Error", message: "invalid ip V4 format." })
    else Gateway.findByIdAndUpdate({ _id: req.params.gatewayId }, req.body, { new: true })
      .then(result => {
        if (result)
          res.status(201).send({ status: 'success', result: result });
